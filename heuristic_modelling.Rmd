...
[@Ferguson2006]. Spatial structure is also modelled explicitly. These complex models aim to make more reliable predictions than the SIR through being much more realistic. However they can suffer from the "42" problem. The number 42 is well known to those familiar with the work of Douglas Adams. It is the ultimate resolution of a computer model aimed at resolving all doubts regarding life the universe and everything. However as deep thought ran undocumented code and did not keep track of its working it was unable to reveal why 42 was the answer. 
...
Gupta refers to beta as a "jumble of parameters". This is a very good way of looking at the model. Transmission is actually dependent on a complex process of involving the number of [contacts between individuals and the consequence of contact](#networks) . 
...
## Implementing the SIR in R with Johns Hopkins data as input and reference
...
Code for the shiny app is available here. 
...
The equations is solved for the initial starting conditions and parameters chosen by the user.
